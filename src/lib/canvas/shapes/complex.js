...
export const drawPlatonicSolid = (ctx, size, type, config = {}) => {
  ...
  const { vertices, faces } = ShapeConfigs.platonic[type];
  const radius = size / 2;
...
export const drawFibonacciSpiral = (ctx, size, config = {}) => {
  const finalConfig = { ...defaultShapeConfig, ...ShapeConfigs.fibonacci, ...config };
  applyTransforms(ctx, size, finalConfig);
...
export const complexShapes = {
  platonicSolid: (ctx, size, type, config) => drawPlatonicSolid(ctx, size, type, config),
  fibonacciSpiral: (ctx, size, config) => drawFibonacciSpiral(ctx, size, config),
